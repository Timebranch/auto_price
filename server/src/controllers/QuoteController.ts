...
import { db, QuoteTemplate, QuoteRecord } from '../models/database';
...
export class QuoteController {
  ...
  // 创建新的报价记录
  async createQuoteRecord(req: Request, res: Response) {
    try {
      ...
      db.run(sql, [template_id, userId, title, JSON.stringify(formDataObj)], function(err) {
        if (err) {
        ...
        // 返回新创建的记录
        db.get('SELECT * FROM quote_records WHERE id = ?', [this.lastID], (err, row) => {
          if (err) {
  ...
  // 获取所有报价记录
  async getQuoteRecords(req: Request, res: Response) {
    try {
      const authReq = req as any;
  ...
  // 获取指定报价记录
  async getQuoteRecord(req: Request, res: Response) {
    try {
      const { id } = req.params;
      const authReq = req as any;
      const user = authReq.user;
  ...
  // 更新报价记录
  async updateQuoteRecord(req: Request, res: Response) {
    try {
      const { id } = req.params;
      ...
      db.run(sql, [title, JSON.stringify(form_data), status, id], function(err) {
        if (err) {
        ...
        // 返回更新后的记录
        db.get('SELECT * FROM quote_records WHERE id = ?', [id], (err, row) => {
          if (err) {
  ...
  // 删除报价记录
  async deleteQuoteRecord(req: Request, res: Response) {
    try {
      const { id } = req.params;
      
      // 先获取记录信息，用于删除相关文件
      db.get('SELECT * FROM quote_records WHERE id = ?', [id], (err, row: any) => {
        if (err) {
          console.error('获取报价记录失败:', err);
          return res.status(500).json({ error: '获取报价记录失败' });
        }
        ...
        // 删除数据库记录
        db.run('DELETE FROM quote_records WHERE id = ?', [id], function(err) {
          if (err) {
          ...
          try {
            ...
            if (row.generated_pdf_path) {
              ...
              const raw: string = String(row.generated_pdf_path);
              ...
              if (targetPath) {
                try { fs.unlinkSync(targetPath); } catch (fileErr) { console.error('删除PDF文件失败:', fileErr); }
              }
            }
          } catch (resolveErr) {
  ...
  async generatePDF(req: Request, res: Response) {
    try {
      ...
      db.get(sql, [id], async (err, row: any) => {
        if (err) {
          console.error('获取报价记录失败:', err);
          return res.status(500).json({ error: '获取报价记录失败' });
        }
        ...
        try {
          ...
          try {
            ...
            const uploadsDirCandidates = [
              path.join(__dirname, '../uploads'),      // dist/index.ts 场景（__dirname 指向 dist）
              path.join(__dirname, '../../uploads'),   // src/controllers 场景（__dirname 指向 src/controllers）
              path.join(process.cwd(), 'uploads')      // 兜底：当前工作目录下的 uploads
            ];
            ...
            if (publicPath) {
              const v = Date.now();
              (formData as any).companyTitleImageUrl = `${hostUrl}${publicPath}?v=${v}`;
            }
            ...
            const logoJpgPath = path.join(uploadsDir, 'logo.jpg');
            {
              ...
              if (fs.existsSync(logoNewPath)) {
                (formData as any).companyLogoUrl = `${hostUrl}/uploads/logo_new.png?v=${v}`;
              } else if (fs.existsSync(logoJpgPath)) {
                (formData as any).companyLogoUrl = `${hostUrl}/uploads/logo.jpg?v=${v}`;
              }
            }
            // 签章图片：优先内嵌 base64，避免打印引擎对外链图片的裁剪或加载问题
            try {
              const v = Date.now();
              const currentStamp: string | undefined = (formData as any).stampImageUrl;
              const isDefaultPath = (url: string) => /\/uploads\/qianzhang\.png/.test(url || '');
              const stampPngPath = path.join(uploadsDir, 'qianzhang.png');
              const canInlineDefault = fs.existsSync(stampPngPath) && (!currentStamp || isDefaultPath(currentStamp));
              if (canInlineDefault) {
                const buf = fs.readFileSync(stampPngPath);
                (formData as any).stampImageUrl = `data:image/png;base64,${buf.toString('base64')}`;
              } else {
                ...
                if (!currentStamp || typeof currentStamp !== 'string') {
                  (formData as any).stampImageUrl = defaultStamp;
                } else if (isDefaultPath(currentStamp) && !/[?&]v=/.test(currentStamp)) {
                  (formData as any).stampImageUrl = currentStamp + (currentStamp.includes('?') ? '&' : '?') + `v=${v}`;
                }
              }
            } catch (stampErr) {
            ...
            try {
              const v = Date.now();
              const rawUrl: string | undefined = (formData as any).itemsImageUrl;
              if (rawUrl && typeof rawUrl === 'string') {
                ...
                (formData as any).itemsImageUrl = finalUrl;
                (formData as any).preferManualTotals = true;
              }
            } catch (itemsImgErr) {
          ...
          try {
            ...
            const candidatePaths: string[] = [
              ...
              path.join(__dirname, '../templates/custom_template.html'),
              // dist/controllers -> ../../src/templates
              path.join(__dirname, '../../src/templates/custom_template.html'),
              // 工作目录（server）兜底
              path.join(process.cwd(), 'src/templates/custom_template.html')
            ];
            let selectedPath: string | null = null;
            for (const p of candidatePaths) {
              try {
                if (fs.existsSync(p)) { selectedPath = p; break; }
              } catch { /* ignore */ }
            }

            if (selectedPath) {
              const fileContent = fs.readFileSync(selectedPath, 'utf-8');
          ...
          const createdAtRaw: string = String(row.created_at || '').trim();
          let createdStr = '';
          try {
            if (createdAtRaw) {
              // 仅保留日期部分 YYYY-MM-DD
          ...
          const companyRaw = String((formData as any).customerName || (formData as any).companyName || row.title || '报价单').trim();
          ...
          db.run(
            'UPDATE quote_records SET generated_pdf_path = ?, status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
            [relativePublicPath, 'completed', id],
            (updateErr) => {
              if (updateErr) {
  ...
  async downloadPDF(req: Request, res: Response) {
    try {
      ...
      db.get('SELECT * FROM quote_records WHERE id = ?', [id], (err, row: any) => {
        ...
        let targetPath: string | null = null;
        for (const p of absCandidates) {
          try { if (fs.existsSync(p)) { targetPath = p; break; } } catch { /* ignore */ }
        }
        if (!targetPath) {
          return res.status(404).json({ error: 'PDF文件不存在' });
        }
