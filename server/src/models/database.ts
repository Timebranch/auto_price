...
export interface QuoteTemplate {
  ...
  fields: string; // JSON字符串，存储表单字段配置
  created_by: number; // 关联用户ID
  created_at: string;
  updated_at: string;
}
...
export interface QuoteRecord {
  ...
  title: string;
  form_data: string; // JSON字符串，存储表单填写的数据
...
export interface SettlementRecord {
  id: number;
  delivery_id: number; // 唯一关联发货单
  order_name: string;
  customer_name: string;
  delivery_date: string; // 文本日期用于展示与搜索
  address?: string;
  shipper_name?: string;
  created_by: number;
  items: string; // JSON 字符串，保存明细快照
  total_price: number;
  created_at: string;
  updated_at: string;
}
...
export interface TechnicalTask {
  ...
  deliverables: string; // JSON 数组字符串
    ...
    `, (err) => {
      ...
      db.all(`PRAGMA table_info(users)`, (pragmaErr, rows: any[]) => {
        ...
        const ensureNext = () => {
          ...
          db.run(`UPDATE users SET role = 'sales' WHERE role = 'user'`, (roleErr) => {
            if (roleErr) {
              reject(roleErr);
              return;
            }
          ...
          `, (err) => {
            ...
            `, (err) => {
              ...
              // 迁移：如果旧表缺少列，则进行增补
              db.all(`PRAGMA table_info(quote_records)`, (pragmaErr, cols: any[]) => {
                if (pragmaErr) {
                  reject(pragmaErr);
                  return;
                }
                ...
                if (!colNames.includes('user_id')) {
                  tasks.push(() => {
                    db.run(`ALTER TABLE quote_records ADD COLUMN user_id INTEGER`, (alterErr) => {
                      if (alterErr) { reject(alterErr); return; }
                      // 将历史数据归属管理员用户（如存在）
                      db.get(`SELECT id FROM users WHERE username = ?`, ['admin'], (getErr, row) => {
                        if (getErr) { reject(getErr); return; }
                        const adminId = (row as any)?.id || null;
                        if (adminId) {
                          db.run(`UPDATE quote_records SET user_id = ? WHERE user_id IS NULL`, [adminId], (updErr) => {
                            if (updErr) { reject(updErr); return; }
                            runNextTask();
                          });
                        } else {
                          runNextTask();
                ...
                if (!colNames.includes('generated_pdf_path')) {
                  tasks.push(() => {
                    db.run(`ALTER TABLE quote_records ADD COLUMN generated_pdf_path TEXT`, (alterErr) => {
                      if (alterErr) { reject(alterErr); return; }
                      runNextTask();
                    });
                  });
                ...
                if (!colNames.includes('status')) {
                  tasks.push(() => {
                    db.run(`ALTER TABLE quote_records ADD COLUMN status TEXT DEFAULT 'draft'`, (alterErr) => {
                      if (alterErr) { reject(alterErr); return; }
                      runNextTask();
                    });
                  });
                ...
                if (!colNames.includes('created_at')) {
                  tasks.push(() => {
                    db.run(`ALTER TABLE quote_records ADD COLUMN created_at DATETIME DEFAULT CURRENT_TIMESTAMP`, (alterErr) => {
                      if (alterErr) { reject(alterErr); return; }
                      runNextTask();
                    });
                  });
                ...
                if (!colNames.includes('updated_at')) {
                  tasks.push(() => {
                    db.run(`ALTER TABLE quote_records ADD COLUMN updated_at DATETIME DEFAULT CURRENT_TIMESTAMP`, (alterErr) => {
                      if (alterErr) { reject(alterErr); return; }
                      runNextTask();
                    });
                  });
                ...
                const runNextTask = () => {
                  if (tasks.length === 0) {
                    ...
                    `, (deliveryErr) => {
                      if (deliveryErr) { reject(deliveryErr); return; }
                      // 迁移：补充发货单额外字段
                      db.all(`PRAGMA table_info(delivery_notes)`, (pragmaErr2, cols2: any[]) => {
                        if (pragmaErr2) { reject(pragmaErr2); return; }
                        const names = (cols2 || []).map((c:any) => c.name);
                        const alters: Array<() => void> = [];
                        const addCol = (def: string) => () => db.run(`ALTER TABLE delivery_notes ADD COLUMN ${def}`,(e)=>{ if(e){ reject(e); return;} runNextAlter(); });
                        if (!names.includes('status')) alters.push(addCol(`status TEXT DEFAULT 'draft'`));
                        if (!names.includes('license_plate')) alters.push(addCol(`license_plate TEXT`));
                        if (!names.includes('driver_name')) alters.push(addCol(`driver_name TEXT`));
                        if (!names.includes('driver_phone')) alters.push(addCol(`driver_phone TEXT`));
                        if (!names.includes('receiver_name')) alters.push(addCol(`receiver_name TEXT`));
                        if (!names.includes('receiver_phone')) alters.push(addCol(`receiver_phone TEXT`));
                        if (!names.includes('shipper_name')) alters.push(addCol(`shipper_name TEXT`));
                        if (!names.includes('shipper_phone')) alters.push(addCol(`shipper_phone TEXT`));
                        if (!names.includes('address')) alters.push(addCol(`address TEXT`));
                        if (!names.includes('note')) alters.push(addCol(`note TEXT`));
                      const runNextAlter = () => {
                        if (alters.length === 0) {
                          // 迁移：将历史状态 'generated' 统一改为 'pending_review'
                          db.run(`UPDATE delivery_notes SET status = 'pending_review' WHERE status = 'generated'`, (mErr) => {
                            ...
                            `, (itemsErr) => {
                              if (itemsErr) { reject(itemsErr); return; }
                              // 迁移 delivery_items：补充材质/单重/总重字段
                              db.all(`PRAGMA table_info(delivery_items)`, (pragmaErr3, cols3: any[]) => {
                                if (pragmaErr3) { reject(pragmaErr3); return; }
                                const names3 = (cols3 || []).map((c:any) => c.name);
                                const alters3: Array<() => void> = [];
                                const addCol3 = (def: string) => () => db.run(`ALTER TABLE delivery_items ADD COLUMN ${def}`,(e)=>{ if(e){ reject(e); return;} runNextAlter3(); });
                                if (!names3.includes('material')) alters3.push(addCol3(`material TEXT`));
                                if (!names3.includes('unit_weight')) alters3.push(addCol3(`unit_weight TEXT`));
                                if (!names3.includes('total_weight')) alters3.push(addCol3(`total_weight TEXT`));
                                const runNextAlter3 = () => {
                                  if (alters3.length === 0) {
                                    ...
                                    `, (settErr) => {
                                      if (settErr) { reject(settErr); return; }
                                      // 创建知识库表
                                      db.run(`
                                      ...
                                      `, (tableErr) => {
                                        if (tableErr) { reject(tableErr); return; }
                                        // 创建技术任务表
                                        db.run(`
...
async function insertDefaultData(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    db.get('SELECT id FROM users WHERE username = ?', ['admin'], (err, row) => {
      ...
      if (row) {
        // 管理员已存在，直接插入模板
        insertDefaultTemplate((row as any).id).then(resolve).catch(reject);
        return;
      }
...
// 插入默认报价单模板
async function insertDefaultTemplate(createdBy: number): Promise<void> {
  return new Promise((resolve, reject) => {
    // 检查是否已存在默认模板
    db.get('SELECT id FROM quote_templates WHERE name = ?', ['默认报价单模板'], (err, row) => {
      if (err) {
        reject(err);
        return;
      }
      ...
      // 如果存在自定义模板文件，则覆盖默认模板
      try {
        const templateFilePath = path.join(__dirname, '../templates/custom_template.html');
        if (fs.existsSync(templateFilePath)) {
          defaultTemplateHtml = fs.readFileSync(templateFilePath, 'utf-8');
        }
      } catch (e) {
      ...
      db.get('SELECT id FROM quote_templates WHERE name = ?', ['默认报价单模板'], (err2, row2) => {
        if (err2) {
          reject(err2)
          return
        }
        if (row2) {
          db.run(
            'UPDATE quote_templates SET description = ?, template_html = ?, fields = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?',
            [
              '适用于常规报价单的模板',
              defaultTemplateHtml,
          ...
          db.run(
            ...
            VALUES (?, ?, ?, ?, ?)
            `,
            ['默认报价单模板', '适用于常规报价单的模板', defaultTemplateHtml, JSON.stringify(defaultFieldsConfig), createdBy],
            function(err3) {
