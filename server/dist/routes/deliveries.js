...
router.get('/', auth_1.authenticateToken, (req, res) => {
  ...
  `;
    ...
    const params = [orderNameLike, customerNameLike];
    // 权限过滤
...
router.get('/:id', auth_1.authenticateToken, (req, res) => {
    ...
    database_1.db.get('SELECT * FROM delivery_notes WHERE id = ?', [id], (err, row) => {
        if (err) {
        ...
        database_1.db.all('SELECT product_name, material, spec, length, quantity, unit, unit_weight, total_weight, remark, sort_order FROM delivery_items WHERE delivery_id = ? ORDER BY sort_order ASC, id ASC', [id], (itemsErr, itemsRows) => {
            if (itemsErr) {
...
router.get('/:id/download', auth_1.authenticateToken, (req, res) => {
    ...
    database_1.db.get('SELECT * FROM delivery_notes WHERE id = ?', [id], (err, row) => {
        ...
        const resolveAbsolutePath = (fp) => (path_1.default.isAbsolute(fp)
            ? fp
            : path_1.default.join(__dirname, '../../', fp.replace(/^\/?/, '')));
        // 若已有文件路径且文件存在，直接下载
        if (filePath) {
            const absolutePath = resolveAbsolutePath(filePath);
            if (fs_1.default.existsSync(absolutePath)) {
                return res.download(absolutePath, path_1.default.basename(absolutePath));
            }
        }
        // 否则：按模板即时生成PDF，保存并回填 file_path 后再下载
        const releaseOrNullPromise = PdfMutex_1.pdfMutex.acquire();
        releaseOrNullPromise.then(async (release) => {
            try {
                // 读取发货单明细
                const items = await new Promise((resolveItems, rejectItems) => {
                    database_1.db.all('SELECT product_name, material, spec, length, quantity, unit, unit_weight, total_weight, remark, sort_order FROM delivery_items WHERE delivery_id = ? ORDER BY sort_order ASC, id ASC', [id], (itemsErr, rows) => {
                        if (itemsErr)
                            return rejectItems(itemsErr);
                        resolveItems(rows || []);
                    });
                });
                // 选择模板路径（兼容 src 与 dist）
                const templateCandidates = [
                    path_1.default.join(__dirname, '../templates/delivery_note_template.html'),
                    path_1.default.join(__dirname, '../../templates/delivery_note_template.html')
                ];
                let templateHtml = '';
                for (const p of templateCandidates) {
                    try {
                        if (fs_1.default.existsSync(p)) {
                            templateHtml = fs_1.default.readFileSync(p, 'utf-8');
                            break;
                        }
                    }
                    catch { /* ignore */ }
                ...
                const uploadsDir = uploadsDirCandidates.find(p => { try {
                ...
                const stampPath = path_1.default.join(uploadsDir, 'qianzhang.png');
                const companyLogoUrl = fs_1.default.existsSync(logoNewPath)
                    ? `${hostUrl}/uploads/logo_new.png`
                    : (fs_1.default.existsSync(logoJpgPath) ? `${hostUrl}/uploads/logo.jpg` : undefined);
                const stampImageUrl = fs_1.default.existsSync(stampPath) ? `${hostUrl}/uploads/qianzhang.png` : undefined;
                // 构造模板数据
                const deliveryDate = String(row.delivery_time || '').slice(0, 10);
                const data = {
                    ...
                    delivery_items: items.map((it, idx) => ({
                        index: idx + 1,
                        product_name: it.product_name || '',
                        material: it.material || '',
                        spec: it.spec || '',
                ...
                try {
                    const minRows = 12;
                    const baseItems = Array.isArray(data.delivery_items)
                        ? data.delivery_items.slice()
                        : [];
                    if (baseItems.length < minRows) {
                        ...
                        data.delivery_items_padded = [...baseItems, ...placeholders].map((item, idx) => ({
                            ...item,
                            index: idx + 1
                        }));
                    }
                    else {
                        data.delivery_items_padded = baseItems.map((item, idx) => ({
                            ...item,
                            index: idx + 1
                        }));
                    }
                ...
                const generator = new PDFGenerator_1.PDFGenerator();
                ...
                await new Promise((resolveUpdate, rejectUpdate) => {
                    database_1.db.run('UPDATE delivery_notes SET file_path = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?', [relativePublicPath, id], function (updateErr) {
                        if (updateErr)
                            return rejectUpdate(updateErr);
                        resolveUpdate(true);
                    });
                });
                return res.download(pdfAbsPath, path_1.default.basename(pdfAbsPath));
            }
            catch (genErr) {
...
router.post('/', auth_1.authenticateToken, (req, res) => {
    ...
    const { order_name, customer_name, delivery_date, // YYYY-MM-DD（来自日期组件）
    delivery_time, // 可选：完整日期时间
    items, license_plate, driver_name, driver_phone, receiver_name, receiver_phone, shipper_name, shipper_phone, address, note, status, // 'draft' | 'generated'
     } = req.body || {};
    // 顶层必填校验
    const errs = [];
    if (!order_name)
    ...
    if (!customer_name)
        errs.push('客户名称必填');
    const dt = (delivery_time || (delivery_date ? `${delivery_date} 00:00:00` : ''));
    if (!dt)
        errs.push('发货日期必填');
    if (!license_plate)
        errs.push('车牌号必填');
    ...
    if (itemsArr.length < 1)
        errs.push('至少需要一组商品条目');
    itemsArr.forEach((it, idx) => {
        if (!it?.product_name)
            errs.push(`第${idx + 1}行商品名称必填`);
        if (!it?.spec)
            errs.push(`第${idx + 1}行规格必填`);
        // 长度非必填
        if (it?.quantity == null || it?.quantity === '')
            errs.push(`第${idx + 1}行数量必填`);
        if (!it?.unit)
            errs.push(`第${idx + 1}行单位必填`);
    });
    if (errs.length > 0) {
    ...
    database_1.db.run(insertSql, [
      ...
      `;
        database_1.db.serialize(() => {
            let anyErr = null;
            itemsArr.forEach((it, idx) => {
                database_1.db.run(insertItemSql, [
                    deliveryId,
                    it.product_name,
                    it.material || null,
                    it.spec,
...
router.patch('/:id/status', auth_1.authenticateToken, (req, res) => {
    ...
    database_1.db.run(`UPDATE delivery_notes SET status = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?`, [nextStatus, id], function (err) {
        if (err) {
