...
router.get('/', auth_1.authenticateToken, (req, res) => {
    ...
    const shipperName = req.query.shipperName || '';
    const sql = `
    SELECT 
      id,
      delivery_id,
...
router.get('/:id', auth_1.authenticateToken, (req, res) => {
    ...
    database_1.db.get('SELECT * FROM settlements WHERE id = ?', [id], (err, row) => {
        if (err) {
...
// 创建/覆盖结算单（仅管理员/财务）
router.post('/upsert', auth_1.authenticateToken, (req, res) => {
    const role = req.user?.role;
    const userId = req.user?.id;
    if (role !== 'admin' && role !== 'finance') {
        return res.status(403).json({ error: '无权生成结算单' });
    }
    const { delivery_id, order_name, customer_name, delivery_date, address, shipper_name, items, total_price } = req.body || {};
  ...
  `;
    ...
    database_1.db.run(sql, params, function (err) {
        ...
        database_1.db.get('SELECT id FROM settlements WHERE delivery_id = ?', [delivery_id], (getErr, row) => {
            if (getErr) {
...
router.get('/:id/download', auth_1.authenticateToken, (req, res) => {
    ...
    database_1.db.get('SELECT * FROM settlements WHERE id = ?', [id], async (err, row) => {
        if (err) {
            return res.status(500).json({ error: '查询结算单失败' });
        }
        if (!row) {
        ...
        try {
            ...
            const templateCandidates = [
                path_1.default.join(__dirname, '../templates/settlement_template.html'),
                path_1.default.join(__dirname, '../../templates/settlement_template.html')
            ];
            let templateHtml = '';
            ...
            const padded = (baseItems.length < minRows)
                ...
                : baseItems;
            const settlement_items_padded = padded.map((it, idx) => ({
                index: idx + 1,
